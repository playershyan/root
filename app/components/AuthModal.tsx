...
export default function AuthModal({ isOpen, onClose, initialAuthType = 'login', returnUrl, showEmailLogin = false }: AuthModalProps) {
  ...
  const formatPhoneNumber = (value: string) => {
    ...
    if (numbers.startsWith(dialCode)) {
      return '+' + numbers.slice(0, dialCode.length + 9)
    } else if (selectedCountry.code === 'LK' && numbers.startsWith('0')) {
      return '+' + dialCode + numbers.slice(1, 10)
    } else {
      return '+' + dialCode + numbers.slice(0, 9)
    }
  }
    ...
    if (mode === 'phone') {
      ...
      if (authType === 'register') {
        ...
        if (result.success) {
          ...
          // Use returnUrl prop first, then sessionStorage fallback, then default
          const pendingRedirect = sessionStorage.getItem('pendingRedirect')
          const redirectTo = returnUrl || pendingRedirect || '/profile'
          
          // Clear the pending redirect after using it
          if (pendingRedirect) {
        ...
        // Log what's in localStorage BEFORE login
        console.log('[AuthModal] Before email login - localStorage pendingRedirect:', localStorage.getItem('pendingRedirect'))
        console.log('[AuthModal] returnUrl prop:', returnUrl)
            ...
            <div className="space-y-4">
              <div>
                <h2 className="text-2xl font-bold text-blue-600 mb-1">
                  {authType === 'login' ? 'Welcome Back to VERA' : 'Join VERA Today'}
                </h2>
                <p className="text-gray-600 text-sm">
                  {authType === 'login' ? 'Sign in to manage your listings, save favorites, and connect with buyers and sellers.' : 'Create your account to start buying and selling vehicles with Sri Lanka\'s leading automotive marketplace.'}
                </p>
              </div>
              ...
              {authType === 'register' && (
                <div className="grid grid-cols-1 gap-4">
                  <div className="bg-blue-50 p-4 rounded-lg">
                    ...
                    <h3 className="font-semibold text-gray-900 mb-1 text-sm">List Your Vehicles</h3>
                    <p className="text-xs text-gray-600">Post unlimited vehicle ads with AI-powered descriptions and reach thousands of buyers.</p>
                  </div>
                  <div className="bg-green-50 p-4 rounded-lg">
                    ...
                    <h3 className="font-semibold text-gray-900 mb-1 text-sm">Find Your Dream Car</h3>
                    <p className="text-xs text-gray-600">Browse verified listings, save favorites, and post wanted requests to find exactly what you need.</p>
                  </div>
                  <div className="bg-purple-50 p-4 rounded-lg">
                    ...
                    <h3 className="font-semibold text-gray-900 mb-1 text-sm">Connect Safely</h3>
                    <p className="text-xs text-gray-600">Message sellers directly, schedule viewings, and complete transactions with confidence.</p>
                  </div>
                </div>
              )}
              ...
              {authType === 'login' && (
                <ul className="space-y-3">
                  ...
                  <li className="flex items-start gap-3 pb-3 border-b border-gray-100">
                    ...
                    <span className="text-sm text-gray-700">Discover & manage VERA® membership plans.</span>
                  </li>
              ...
              <div className="md:hidden bg-blue-50 p-4 rounded-lg mb-6">
                ...
                <ul className="space-y-2">
                  <li className="flex items-center gap-2 text-xs text-blue-700">
                    ...
                    <span>Post unlimited vehicle ads with AI-powered descriptions</span>
                  </li>
                  <li className="flex items-center gap-2 text-xs text-blue-700">
                    <div className="w-1.5 h-1.5 bg-blue-600 rounded-full flex-shrink-0"></div>
                    <span>Browse verified listings & save favorites</span>
                  </li>
                  <li className="flex items-center gap-2 text-xs text-blue-700">
                    <div className="w-1.5 h-1.5 bg-blue-600 rounded-full flex-shrink-0"></div>
                    <span>Message sellers directly & schedule viewings</span>
                  </li>
                  <li className="flex items-center gap-2 text-xs text-blue-700">
                    <div className="w-1.5 h-1.5 bg-blue-600 rounded-full flex-shrink-0"></div>
                    <span>Complete transactions with confidence</span>
                  </li>
                </ul>
              </div>
            )}
                  ...
                  <div className="mb-6">
                    ...
                    <div className="flex">
                      ...
                      <input
                        ...
                        placeholder={selectedCountry.code === 'LK' ? '771234567' : 'Phone number'}
                        className="flex-1 px-4 py-2 border border-gray-300 border-l-0 rounded-r-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent focus:outline-none"
                        required
                        maxLength={9}
                      />
                    </div>
              ...
              <div>
                <p className="text-center text-gray-600 mb-4">
                  {mode === 'email-otp' ? 
                    ...
                    `We've sent a code to +${selectedCountry.dialCode} ${phone}`
                  }
                </p>
