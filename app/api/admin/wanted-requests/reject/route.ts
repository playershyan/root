...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // Update wanted request with rejection
    const { error: updateError } = await supabase
      ...
      .update({
        ...
        deletion_reason: `Rejected by admin: ${reason}`
      })
      .eq('id', requestId)
    ...
    if (wantedRequest.user_id) {
      const { error: notificationError } = await supabase
        ...
        .insert({
          ...
          type: 'wanted_request_rejected',
          title: 'Wanted Request Rejected',
          message: `Your wanted request "${wantedRequest.title}" has been rejected. Reason: ${reason}`,
    ...
    const { error: logError } = await supabase.rpc('log_admin_activity', {
      ...
      p_action_type: 'reject_wanted_request',
      p_details: {
    ...
    return NextResponse.json({
      success: true,
      message: 'Wanted request rejected successfully'
    })
