...
import { MapPin, Calendar, Star, MessageCircle, Eye, AlertTriangle, TrendingUp } from 'lucide-react'
...
export default function GoldFeaturedWantedCard({ request, size = 'regular' }: GoldFeaturedWantedCardProps) {
  ...
  return (
    <div className={`
      ...
      ${isLarge ? 'hover:scale-[1.01]' : 'hover:-translate-y-1'}
    `}>
      {/* Shimmer Effect */}
      ...
      <div className="absolute top-0 left-0 right-0 p-4 flex items-start justify-between z-20">
        ...
        {/* Urgent Badge */}
        {request.is_urgent && (
          <div className="bg-gradient-to-r from-red-500 to-red-600 text-white px-3 py-1.5 rounded-full shadow-lg flex items-center gap-1.5 animate-pulse">
      ...
      <Link href={`/wanted/${request.id}`} className="block">
        <div className={`relative pt-14 pb-6 px-6 ${isLarge ? 'lg:px-8' : ''}`}>
          ...
          {/* Budget Section */}
          <div className="bg-gradient-to-br from-amber-100/50 to-yellow-100/50 rounded-xl p-4 mb-4 border border-amber-200/50">
