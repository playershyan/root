-- Migration: Fix Supabase Security Advisor Issues (CORRECTED)
-- Date: 2025-09-08
...
-- reset_unread_count (updated for correct schema)
...
BEGIN
    ...
    SET buyer_unread_count = CASE WHEN buyer_id = p_user_id THEN 0 ELSE buyer_unread_count END,
...
-- update_conversation_on_message (updated for correct schema)
...
-- check_deletion_safety (updated for correct schema)
...
BEGIN
    SELECT enable_safety_checks INTO v_enabled
    ...
    SELECT EXISTS(
        ...
        WHERE items_to_delete @> jsonb_build_array(jsonb_build_object('type', p_item_type, 'id', p_item_id))
        AND status = 'approved'
...
-- increment_listing_views_enhanced (updated for correct schema)
...
BEGIN
    ...
    -- Record the view
    INSERT INTO listing_views (listing_id, user_id, viewer_ip)
...
-- safely_delete_old_records (updated for correct schema)
...
BEGIN
    ...
    SELECT enable_safety_checks, min_delete_age_days 
    INTO v_config_enabled, v_min_age
    FROM deletion_safety_config
    WHERE id = 1;
    ...
    SELECT 'listings', id, row_to_json(l.*), NOW()
    FROM listings l
    WHERE l.deleted_at IS NOT NULL 
    AND l.deleted_at < NOW() - make_interval(days => v_min_age)
    AND NOT EXISTS (
        SELECT 1 FROM deletion_backups db
        WHERE db.table_name = 'listings' AND db.record_id = l.id
    );
    ...
    WHERE deleted_at IS NOT NULL 
    AND deleted_at < NOW() - make_interval(days => v_min_age);
    GET DIAGNOSTICS v_marked_listings = ROW_COUNT;
    
    -- Backup and mark old wanted requests
    INSERT INTO deletion_backups (table_name, record_id, backup_data, deleted_at)
    SELECT 'wanted_requests', id, row_to_json(w.*), NOW()
    FROM wanted_requests w
    WHERE w.deleted_at IS NOT NULL 
    AND w.deleted_at < NOW() - make_interval(days => v_min_age)
    AND NOT EXISTS (
        SELECT 1 FROM deletion_backups db
        WHERE db.table_name = 'wanted_requests' AND db.record_id = w.id
    );
...
-- restore_from_backup (updated for correct schema)
...
BEGIN
    ...
    IF v_backup.table_name = 'listings' THEN
        -- Restore listing
        UPDATE listings
        SET deleted_at = NULL, permanently_deleted = false
        WHERE id = v_backup.record_id;
        v_success := FOUND;
    ELSIF v_backup.table_name = 'wanted_requests' THEN
...
-- restore_user_item (updated for correct schema)
...
BEGIN
    ...
    IF v_success THEN
        INSERT INTO deletion_logs (table_name, record_id, user_id, deletion_reason)
        VALUES (p_item_type, p_item_id, p_user_id, 'User restored item');
    END IF;
